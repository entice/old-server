...
trait CharacterController extends Controller {
  self: Security
    with Characters
    with Attributes =>

  private def getAppearance(f: CharacterCreateForm): Attributes#Appearance = {
    ...
    Appearance(
      ...
      f.sex, f.height, f.skinColor, f.hairColor, f.hairstyle, f.face).asInstanceOf[Attributes#Appearance]
  }
    ...
    authorize match {
      ...
      case Some(client) =>
        action match {
          ...
          case _ => Redirect(entice.server.routes.Global.webLobby()).flashing("message" -> "Action or character name unkown.")
        }
        ...
        action match {
          case "delete" if (client().chars.contains(name)) =>
            characters.deleteByName(name).map { x =>
              ...
              Redirect(entice.server.routes.Global.webLobby()).flashing("message" -> "Character deleted.")
            }
            ...
            charCreateForm.bindFromRequest.fold(
              ...
              charData => {
                ...
                characters.createOrUpdateByName(if (name == "") charData.charName else name, dbCharacter).map { x =>
                  ...
                  Redirect(entice.server.routes.Global.webLobby()).flashing("message" -> s"""Character ${action.stripSuffix("e")}ed""")
                }
              })
          case _ => Future.successful(Redirect(entice.server.routes.Global.webLobby()).flashing("message" -> "Error: Action or character name unkown."))
        }
